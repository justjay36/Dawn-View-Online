  ...
  try{
    ...
    if(validData(saved)){
      ...
      if(previousVersion!==MASTER_SEED.app.version){
        ...
        state.tracking.next_payday=state.income_estimator?.side_next_payday||"";
        state.income_estimator=state.income_estimator||{};
        state.income_estimator.primary_next_payday="";
        state.income_estimator.side_next_payday="";
...
function plannerValues(){
  return {
    primary_rate:Number($("primaryRate").value||0),
    primary_hours:Number($("primaryHours").value||0),
    primary_withholding:Number($("primaryWithholding").value||0),
    primary_payday:$("primaryPayday").value,
    side_rate:Number($("sideRate").value||0),
    side_hours:Number($("sideHours").value||0),
    side_overtime_enabled:$("sideOvertimeEnabled").checked,
    side_overtime_hours:Number($("sideOvertimeHours").value||0),
    side_overtime_multiplier:Number($("sideOvertimeMultiplier").value||1.5),
    side_payday:$("sidePayday").value,
    side_notes:$("sideNotes").value.trim(),
...
function plannerCalc(v=plannerValues()){
  const primaryGross=v.primary_rate*v.primary_hours;
  const primaryNet=Math.max(0,primaryGross-(primaryGross*v.primary_withholding/100));
  const sideRegular=v.side_rate*v.side_hours;
  const sideOvertimeRate=v.side_rate*Math.max(1,v.side_overtime_multiplier||1.5);
  const sideOvertime=v.side_overtime_enabled?sideOvertimeRate*v.side_overtime_hours:0;
  const sideNet=sideRegular+sideOvertime;
  const total=primaryNet+sideNet;
  ...
  return {v,primaryNet,sideNet,sideRegular,sideOvertime,sideOvertimeRate,total,gap,restore,remaining};
}
...
function renderPlanner(){
  ...
  $("primaryRate").value=e.primary_rate||0;$("primaryHours").value=e.primary_hours||0;$("primaryWithholding").value=e.primary_withholding_percent||0;$("primaryPayday").value=e.primary_next_payday||"";
  $("sideRate").value=e.side_net_hourly_rate||0;$("sideHours").value=e.side_hours_this_week||0;$("sideOvertimeEnabled").checked=!!e.side_overtime_enabled;$("sideOvertimeHours").value=e.side_overtime_hours||0;$("sideOvertimeMultiplier").value=e.side_overtime_multiplier||1.5;$("sidePayday").value=e.side_next_payday||"";$("sideNotes").value=e.side_notes||"";toggleOvertimeFields();
  displayPlanner(plannerCalc());
}
function displayPlanner(c){
  $("plannerMetrics").innerHTML=[["Primary Job estimate",money(c.primaryNet)],["Side Work estimate",money(c.sideNet)],["Combined estimate",money(c.total)],["Next side-work payday",c.v.side_payday||"Not set"],["Next Primary Job payday",c.v.primary_payday||"Not set"]].map(x=>`<div class="metric"><small>${x[0]}</small><strong>${x[1]}</strong></div>`).join("");
  const rows=[];
  if(c.v.side_payday) rows.push(`<div class="paycheck"><div class="payday">Tue</div><div><strong>Side Work</strong><br><small>${c.v.side_payday} • ${c.v.side_hours} regular${c.v.side_overtime_enabled?` + ${c.v.side_overtime_hours} OT @ ${money(c.sideOvertimeRate)}/hr`:""}</small></div><strong>${money(c.sideNet)}</strong></div>`);
  if(c.v.primary_payday) rows.push(`<div class="paycheck"><div class="payday">Wed</div><div><strong>Primary Job</strong><br><small>${c.v.primary_payday} • ${c.v.primary_hours} hours</small></div><strong>${money(c.primaryNet)}</strong></div>`);
...
async function savePlannerDefaults(){
  ...
  Object.assign(e,{primary_rate:v.primary_rate,primary_hours:v.primary_hours,primary_withholding_percent:v.primary_withholding,primary_next_payday:v.primary_payday,side_net_hourly_rate:v.side_rate,side_hours_this_week:v.side_hours,side_overtime_enabled:v.side_overtime_enabled,side_overtime_hours:v.side_overtime_hours,side_overtime_multiplier:v.side_overtime_multiplier,side_next_payday:v.side_payday,side_notes:v.side_notes});
...
async function createExpectedPaychecks(){
  ...
  upsert("Side Work",v.side_payday,c.sideNet);upsert("Primary Job",v.primary_payday,c.primaryNet);
  state.tracking.paycheck_records=records;
  state.tracking.next_payday=[v.side_payday,v.primary_payday].filter(Boolean).sort()[0]||"";
...
function toggleOvertimeFields(){
  const enabled=$("sideOvertimeEnabled")?.checked;
  $("sideOvertimeFields")?.classList.toggle("disabled-fields",!enabled);
  $("sideOvertimeHours").disabled=!enabled;
  $("sideOvertimeMultiplier").disabled=!enabled;
}
...
function syncGoogleCalendar(){
  const c=calendarState();
  if(!window.AndroidCalendar?.syncGoogleCalendar){c.status="error";c.message="Calendar sync is not active in DawnView Online yet.";renderCalendarSettings();return;}
...
async function exportBackup(){
  ...
  const filename=`dawnview-online-backup-${localDateKey()}.json`;
...
const PERSONALIZE_CTA_KEY="dawnview-online-personalized";
const LEGACY_PERSONALIZE_CTA_KEYS=["dawnview-online-v0.2-personalized","dawnview-online-v0.3-personalized","dawnview-online-v0.4-personalized","dawnview-online-v0.5-personalized"];
const WELCOME_KEY="dawnview-online-welcome";
const LEGACY_WELCOME_KEYS=["dawnview-online-v0.2-welcome","dawnview-online-v0.3-welcome","dawnview-online-v0.4-welcome","dawnview-online-v0.5-welcome"];

function personalizedOnThisDevice(){
  let personalized=false;
  try{
    personalized=Boolean(localStorage.getItem(PERSONALIZE_CTA_KEY));
    if(!personalized){
      personalized=LEGACY_PERSONALIZE_CTA_KEYS.some(key=>Boolean(localStorage.getItem(key)));
      if(personalized)localStorage.setItem(PERSONALIZE_CTA_KEY,"1");
    }
  }catch{}
  return personalized;
}
function setPersonalizationBannerVisible(visible){
  const banner=$("previewBanner");
  if(banner)banner.classList.toggle("hidden",!visible);
}
function syncMakeItYoursCta(){
  setPersonalizationBannerVisible(!personalizedOnThisDevice());
}
function markPersonalized(){
  try{localStorage.setItem(PERSONALIZE_CTA_KEY,"1");}catch{}
  setPersonalizationBannerVisible(false);
}
...
async function startFresh(skipConfirm=false){
  ...
  renderAll();
  markPersonalized();
  try{localStorage.setItem(WELCOME_KEY,"fresh");}catch{}
...
function showFirstLaunchWelcome(){
  ...
  try{
    ...
    if(!seen){
      seen=LEGACY_WELCOME_KEYS.some(key=>Boolean(localStorage.getItem(key)));
      if(seen)localStorage.setItem(WELCOME_KEY,"seen");
    }
  }catch{}
  if(!seen&&typeof dialog.showModal==="function")dialog.showModal();
}
...
function bind(){
  ...
  $("sideOvertimeEnabled").onchange=()=>{toggleOvertimeFields();displayPlanner(plannerCalc());};
  ...
  $("resetBtn").onclick=async()=>{if(confirm("Reset this device to the sample preview data?")){state=clone(MASTER_SEED);await persist();try{localStorage.removeItem(PERSONALIZE_CTA_KEY);LEGACY_PERSONALIZE_CTA_KEYS.forEach(key=>localStorage.removeItem(key));}catch{}setPersonalizationBannerVisible(true);renderAll();}};const freshBtn=$("freshStartBtn");if(freshBtn)freshBtn.onclick=()=>startFresh(false);
  const makeItYoursBtn=$("makeItYoursBtn");if(makeItYoursBtn)makeItYoursBtn.onclick=()=>{markPersonalized();openView("settings");};
  const exploreDemoBtn=$("exploreDemoBtn");if(exploreDemoBtn)exploreDemoBtn.onclick=()=>{try{localStorage.setItem(WELCOME_KEY,"demo");}catch{}$("welcomeDialog")?.close();};
